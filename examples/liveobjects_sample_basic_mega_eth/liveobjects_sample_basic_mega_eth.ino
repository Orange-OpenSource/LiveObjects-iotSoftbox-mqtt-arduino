...
IPAddress ip(172, 16, 25, 241);

EthernetClient client;

static const char* appv_version   = "ARDUINO_MEGA BASIC SAMPLE V03.02";
...
char    appv_status_message[10] = "READY";
...
#if 1
  if (appv_log_level > 0) {
    PRINTF("%lu:appli_sched: %u -- cnt=%u" MEOL, millis(), loop_cnt, appv_measures_counter);
    PRINTF(" -- cnt=%u" MEOL, appv_measures_counter);
    PRINTF(" -- temp=%d" MEOL, appv_measures_temp);
    PRINTF(" -- volt=%2.4f %g" MEOL, appv_measures_volt, appv_measures_volt);
    Serial.print(" ** volt= ");
...
boolean  liveobjects_do_init(void* ctx)
{
  ...
  if (ret < 0) PRINTF("liveobjects_do_init: LiveObjectsClient_AttachResources -> ERROR (%d)", ret);
...
boolean liveobjects_do_connect(void* ctx)
{
  ...
  // -----------------------------------
  Serial.println(F("liveobjects_do_connect: LiveObjectsClient_Connect ..."));
...
#if 0
  Serial.print(millis());
  Serial.println(F(":pause 1000 ms..."));
  delay(1000);
#endif
  net_connect();

  Serial.print(millis());
  Serial.println(F(":loop: CONNECTED => liveobjects_do_init ..."));
  liveobjects_do_init(NULL);
...
{
  int ret;
  unsigned long tf = millis() + 2000;

  if (appv_log_level > 2)  {
    Serial.print(millis());
    Serial.println(F(":nativeLoop ..."));
    Serial.flush();
  }
  ...
  if (appv_con_liveobjects == 2) {
    if (appv_log_level > 1) {
      Serial.print(millis());
      Serial.println(F(":nativeLoop: Call LiveObjectsClient_Cycle(2000) ..."));
    }
    ret = LiveObjectsClient_Cycle(2000);
    if (ret) {
      Serial.print(millis());
      Serial.println(F(":nativeLoop: ERROR returned by LiveObjectsClient_Cycle"));
      appv_con_liveobjects = 3;
    }
  }
  else {
    Serial.print(millis());
    Serial.println(F(":nativeLoop: unexpected connection state "));
...
#if 0
  Serial.print(millis());
  Serial.println(F(":nativeLoop: Done"));
#endif
  Serial.flush();
  return true;
}
...
void loop()
{
  ...
  if (appv_con_liveobjects == 2) {
    ...
    Serial.print(millis());
    Serial.println(F(":loop: call liveobjects_do_connect ..."));
    liveobjects_do_connect(NULL);
  }
  else if ((appv_con_liveobjects == 1)) {
    delay(1000);
  }
