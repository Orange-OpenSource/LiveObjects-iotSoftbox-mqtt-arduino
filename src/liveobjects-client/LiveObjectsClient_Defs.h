...
typedef enum {
	...
	LOD_TYPE_UINT32,      /*!< 32-bit Unsigned intege */
...
typedef struct {
	...
	int32_t cmd_cid;       /*!< Correlation Id */
...
typedef struct {
	...
	int32_t cmd_cid;       /*!< Correlation Id (required to set in command response) */
 ...
 */
typedef int (*LiveObjectsD_CallbackParams_t)(const LiveObjectsD_Param_t* param_ptr, const void* val_ptr, int val_len);
 ...
 */
typedef int (*LiveObjectsD_CallbackCommand_t)(LiveObjectsD_CommandRequestBlock_t* pCmdReqBlk);
 ...
 */
typedef LiveObjectsD_ResourceRespCode_t (*LiveObjectsD_CallbackResourceNotify_t)(uint8_t state,
		const LiveObjectsD_Resource_t* rsc_ptr, const char* version_old, const char* version_new, uint32_t rsc_size);
 ...
 *
 */
typedef int (*LiveObjectsD_CallbackResourceData_t)(const LiveObjectsD_Resource_t* rsc_ptr, uint32_t rsc_offset);
